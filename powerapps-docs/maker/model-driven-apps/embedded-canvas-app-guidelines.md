...
-	You can only enable three embedded canvas apps per form. 
     - You can have multiple embedded canvas apps added to the form but can only enable three at a time.
     - If you try to enable more than three embedded canvas app on a model-driven form you will get a message that reads “A maximum of three canvas apps can be enabled on a form.”
